...
import { ArrowLeft, Wand2, Download, Copy, CheckCircle2, Loader2, Edit3, Sparkles, Save, X } from 'lucide-react';
...
export function GeradorProposta() {
  ...
  // Estados do novo Modal de Improve
  const [isImproveModalOpen, setIsImproveModalOpen] = useState(false);
  const [instrucoesUser, setInstrucoesUser] = useState("");

  // Função centralizada para salvar o texto da proposta no banco
  const salvarTextoNoBanco = async (textoParaSalvar: string) => {
    if (edital && edital.id) {
      try {
        await api.put(`/licitacoes/${edital.id}/proposta`, { 
          textoProposta: textoParaSalvar 
        });
        console.log("Texto salvo no banco com sucesso!");
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // Se a licitação já veio com texto do banco (porque foi acessada pela aba Gestão),
    // carrega direto sem chamar a IA de novo.
    ...
    // Se não tem texto, é uma nova geração, então chama a IA
    api.post('/ia/gerar-proposta', edital)
      .then(response => {
        const texto = response.data.propostaText;
  ...
  const handlePrintPDF = () => {
    ...
    // Configurações para um PDF perfeito (formato A4)
    const opt = {
      margin:       15, // Margem em milímetros
      filename:     `Proposta_${edital.orgao.replace(/[^a-z0-9]/gi, '_')}.pdf`, // Nome dinâmico do arquivo
      image:        { type: 'jpeg', quality: 0.98 },
      html2canvas:  { scale: 2, useCORS: true }, // Escala 2 melhora a nitidez da fonte
  ...
  const submitImprove = async () => {
    if (!instrucoesUser.trim()) {
      alert("Por favor, digite o que deseja melhorar.");
      return;
    }
    ...
    setIsImproving(true);
    try {
      const response = await api.post('/ia/melhorar-proposta', { 
        textoAtual: textoEditavel,
        instrucao: instrucoesUser
      });
      const novoTexto = response.data.propostaText;
      ...
      alert("Erro ao ajustar o documento.");
    } finally {
  ...
  const toggleEditMode = () => {
    // Se o usuário está saindo do modo de edição (salvando), mandamos para o banco
    if (isEditing) {
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <style>
        {`
          @media print {
            ...
            #documento-para-imprimir, #documento-para-imprimir * { visibility: visible; }
            #documento-para-imprimir { position: absolute; left: 0; top: 0; width: 100%; padding: 20px; }
      ...
      {isGenerating ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="text-lg font-bold text-brand-dark mb-2">Redigindo proposta nos padrões governamentais...</h3>
          <p className="text-slate-500 text-center max-w-md">
            Cruzando os dados do edital com as leis 14.133 e impostos.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4 no-print">
            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
              <h3 className="font-bold text-brand-dark mb-3 text-sm uppercase tracking-wider">Ações da IA</h3>
              <div className="space-y-2">
                
                <button 
                  onClick={openImproveModal}
                  disabled={isImproving || isEditing}
                ...
                <button 
                  ...
                  className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-md transition-colors font-bold text-sm border ${isEditing ? 'bg-brand-green text-white border-brand-green hover:bg-green-600' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50'}`}
                >
                  {isEditing ? <Save size={16} /> : <Edit3 size={16} />}
                  {isEditing ? 'Salvar Edição' : 'Editar Manualmente'}
                </button>
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
              <h3 className="font-bold text-brand-dark mb-3 text-sm uppercase tracking-wider">Exportação</h3>
              <div className="space-y-2">
                <button 
          ...
          <div className="lg:col-span-2 bg-white p-10 rounded-xl shadow-md border border-slate-200 min-h-[800px] font-serif text-slate-800 relative">
            ...
            {isImproving && (
              <div className="absolute inset-0 bg-white/80 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center z-10">
                 ...
                 <p className="font-bold text-indigo-900">A IA está reescrevendo o documento...</p>
              </div>
            )}

            <div id="documento-para-imprimir" className="max-w-2xl mx-auto">
              {isEditing ? (
                <textarea 
                  value={textoEditavel}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
            ...
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-md font-bold text-brand-dark">Ajustar com IA</h2>
              </div>
              <button onClick={() => setIsImproveModalOpen(false)} className="text-slate-400 hover:text-slate-600 p-1">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-5">
              ...
              <textarea 
                ...
                placeholder="Ex: Altere o valor total para R$ 150.000,00 e adicione uma cláusula afirmando que o frete está incluso."
            ...
            <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              ...
              >
                <Wand2 size={16} /> Aplicar Alterações
              </button>
            </div>
