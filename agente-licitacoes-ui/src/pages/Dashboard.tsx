...
export function Dashboard() {
  ...
  const [propostaSelecionada, setPropostaSelecionada] =
    useState<LicitacaoSalva | null>(null);
  ...
  const propostasGeradas = historico.filter(
    (lic) => lic.propostaGerada === true
  );

  // Função para converter "R$ 450.000,00" em número para o gráfico
  ...
  const valorTotalFunil = historico.reduce(
    (acc, lic) =>
      acc + converterMoeda(lic.valorEstimado),
    0
  );

  const valorTotalPropostas = propostasGeradas.reduce(
    (acc, lic) =>
      acc + converterMoeda(lic.valorEstimado),
    0
  );
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
      ...
      filename: `Proposta_${propostaSelecionada.orgao.replace(
        /[^a-z0-9]/gi,
        '_'
      )}.pdf`,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Métricas de desempenho e histórico de propostas emitidas.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 bg-brand-dark text-white">
          ...
          <p className="text-2xl font-bold">
            {valorTotalPropostas.toLocaleString(
              'pt-BR',
              {
                style: 'currency',
                currency: 'BRL'
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mt-8">
        ...
        {propostasGeradas.length === 0 ? (
          <div className="p-8 text-center text-slate-500">
            ...
            Vá em "Matches da IA" e gere sua primeira proposta!
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            ...
            {propostasGeradas.map((licitacao) => (
              <div
                key={licitacao.id}
                className="p-5 hover:bg-slate-50 transition-colors flex flex-col md:flex-row justify-between items-center gap-4"
              >
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto bg-slate-100 flex-1 flex justify-center">
              ...
              <div className="bg-white shadow-sm border border-slate-200 p-10 w-full max-w-3xl rounded min-h-[600px]">
                ...
                <div
                  id="proposta-view-pdf"
                  className="font-serif text-slate-800 whitespace-pre-wrap leading-relaxed text-justify"
                >
                  {propostaSelecionada.textoProposta
                    ? propostaSelecionada.textoProposta
