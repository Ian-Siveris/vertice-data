...
export function Login() {
  ...
  // Teste de conexão com o backend ao carregar a tela
  useEffect(() => {
    api.get('/status')
      .then(response => {
        console.log(
  ...
  // Função para simular login e redirecionar para a Home
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault(); // Evita recarregamento da página

    // Futuramente: api.post('/login', { email, password })
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <a
                  href="#"
                  className="text-xs text-brand-blue hover:underline font-medium transition-all"
                >
                  Esqueceu a senha?
                </a>
              </div>
