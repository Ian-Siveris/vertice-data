...
export function Home() {
  ...
  const [triando, setTriando] =
    useState(false);

  const [filtroOrdem, setFiltroOrdem] =
    useState('maior_score');

  const [filtroCard, setFiltroCard] =
    useState('todas');
  ...
  const handleTriagemAutomatica =
    async () => {
      ...
      try {
        ...
        alert(
          `Triagem concluída! A IA processou e encontrou ${response.data.processadas} novos editais.`
        );
        ...
        alert(
          'Erro ao executar a triagem automática.'
        );
      } finally {
        setTriando(false);
      }
  ...
  // Filtro duplo:
  // Filtra pela aba selecionada e pelo texto digitado
  const historicoFiltrado =
    historico.filter((licitacao) => {
      // 1. Verifica a aba (card)
      let passaFiltroCard = true;

      if (filtroCard === 'fortes') {
        passaFiltroCard =
          licitacao.scoreIA >= 70;
      }

      if (
        filtroCard === 'aguardando'
      ) {
        passaFiltroCard =
          licitacao.scoreIA >= 50;
      }

      // 2. Verifica a busca por texto
      // (órgão ou objeto)
  ...
  const historicoOrdenado = [
    ...
    if (
      filtroOrdem === 'maior_score'
    ) {
      return b.scoreIA - a.scoreIA;
    }

    if (
      filtroOrdem === 'menor_score'
    ) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Licitações triadas e salvas
            pelo nosso agente autônomo.
          </p>
        </div>

        <button
          onClick={
            handleTriagemAutomatica
          }
          disabled={triando}
          className={`px-4 py-2 rounded-md transition-colors flex items-center gap-2 text-sm font-medium shadow-sm text-white ${
            triando
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div
          ...
          className={`p-4 rounded-lg shadow-sm border transition-all cursor-pointer hover:shadow-md border-b-4 ${
            filtroCard === 'todas'
              ? 'border-brand-blue border-b-brand-blue bg-blue-50'
              : 'border-slate-200 border-b-transparent bg-white'
          }`}
        >
        ...
        <div
          onClick={() =>
            setFiltroCard('fortes')
          }
          className={`p-4 rounded-lg shadow-sm border transition-all cursor-pointer hover:shadow-md border-b-4 ${
            filtroCard === 'fortes'
              ? 'border-brand-green border-b-brand-green bg-green-50'
              : 'border-slate-200 border-b-transparent bg-white'
          }`}
        >
          <p className="text-sm text-slate-500 font-medium">
            Matches Fortes (Score ≥ 70)
          </p>
        ...
        <div
          ...
          className={`p-4 rounded-lg shadow-sm border transition-all cursor-pointer hover:shadow-md border-b-4 ${
            filtroCard ===
            'aguardando'
              ? 'border-amber-400 border-b-amber-400 bg-amber-50'
              : 'border-slate-200 border-b-transparent bg-white'
          }`}
        >
          <p className="text-sm text-slate-500 font-medium">
      ...
      {carregando ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-brand-dark mb-2">
            Nenhum edital analisado
            ainda
          </h3>

          <p className="text-slate-500 mb-4">
            Clique em "Forçar Nova
            Triagem" para a IA buscar
            os editais mais recentes.
          </p>
        </div>
      ) : (
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between bg-slate-50 p-3 rounded-lg border border-slate-200 gap-3">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-md shadow-sm w-full sm:w-auto">
                ...
                >
                  <option value="maior_score">
                    Mais Compatíveis
                    (Maior Score)
                  </option>

                  <option value="menor_score">
                    Menos Compatíveis
                    (Menor Score)
                  </option>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span
                            className={`text-xs font-bold px-2 py-1 rounded tracking-wider uppercase ${
                              licitacao.scoreIA >=
                              70
                    ...
                    <div className="flex flex-wrap items-center justify-between pt-4 border-t border-slate-100 gap-4">
                      ...
                      <div className="flex gap-6">
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-600">
                          ...
                          <span>
                            Abertura:{' '}
                            {licitacao.dataAbertura?.substring(
                              0,
                              10
                            )}
                          </span>
